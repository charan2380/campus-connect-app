...
function ProfilePage() {
  ...
  const [formData, setFormData] = useState({ full_name: '', department: '', year: '', contact_info: '', bio: '', avatar_url: '', college_id_url: '', bus_id_url: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    const promise = new Promise(async (resolve, reject) => {
      try {
        ...
        if (updateError) { reject(updateError); } else { resolve("Profile updated!"); loadProfile(); }
      } catch (error) { reject(error); }
    });
    toast.promise(promise, { loading: 'Saving changes...', success: 'Profile updated!', error: (err) => `Error: ${err.message}` });
    try { await promise; } catch (error) { console.error("Submit Error:", error); }
