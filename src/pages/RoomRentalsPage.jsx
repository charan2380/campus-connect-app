...
// The AddListingModal component is correct and does not need changes.
const AddListingModal = ({ isOpen, onClose, onAddSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (files.length === 0 || !formData.address || !formData.contact_info) {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('room_rentals').insert({
        ...formData,
  ...
  const handleClose = () => {
    setFormData({ address: '', contact_info: '' });
    setFiles([]);
    onClose();
  };
            ...
            <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
              <Dialog.Panel className="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                ...
                <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                  <div><label htmlFor="address" className="block text-sm font-medium text-gray-700">Address</label><textarea name="address" id="address" value={formData.address} onChange={(e) => setFormData({...formData, address: e.target.value})} required rows="2" className="mt-1 block w-full p-2 rounded-md border-gray-300 shadow-sm focus:border-indigo-500"/></div>
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6 py-10">
                      <div className="text-center">
                        ...
                        <div className="mt-4 flex text-sm text-gray-600"><label htmlFor="file-upload" className="relative cursor-pointer rounded-md bg-white font-semibold text-indigo-600"><p>Select files</p><input id="file-upload" name="file-upload" type="file" className="sr-only" onChange={handleFileChange} multiple accept="image/*"/></label></div>
                        <p className="text-xs text-gray-600">{files.length > 0 ? `${files.length} photo(s) selected` : "PNG, JPG up to 10MB each"}</p>
                      </div>
                    </div>
                  </div>
...
// --- THIS IS THE CORRECTED MAIN COMPONENT ---
function RoomRentalsPage() {
  const { getToken } = useAuth();
  const { user } = useUser(); // Changed from userProfile to user for consistency
  ...
  const handleDelete = async (listingId) => {
    ...
    try {
        ...
        // Instead of re-fetching, we can optimistically update the UI for a faster feel
              ...
              {listings.map((listing) => (
                <motion.div key={listing.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.8 }}
                  ...
                  <div className="relative">
                    ...
                    {/* --- 2. THE MISSING DELETE BUTTON, NOW VISIBLE TO OWNER --- */}
                  ...
                  <div className="p-6 flex-grow flex flex-col">
                    ...
                    <div className="border-t mt-4 pt-4 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500">Posted on {new Date(listing.created_at).toLocaleDateString()}</p>
                        </div>
                    </div>
                  </div>
                </motion.div>
