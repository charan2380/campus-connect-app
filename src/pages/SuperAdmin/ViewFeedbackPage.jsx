...
import { Loader2, ArrowLeft, Inbox, User, Trash2, Sparkles, X } from 'lucide-react';
import { Dialog, Transition } from '@headlessui/react';

// --- NEW COMPONENT: SummaryModal (reusable) ---
...
const FeedbackCard = ({ feedback, index, onDelete }) => (
  <motion.li initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}
    ...
    <div className="flex justify-between items-start">
        ...
        <button onClick={() => onDelete(feedback.id)} className="p-1.5 rounded-full text-red-500 hover:bg-red-100"><Trash2 className="h-4 w-4" /></button>
    </div>
    <p className="text-sm text-gray-500 mb-2">Channel: <span className="font-semibold text-gray-700">{feedback.channel}</span> | Target: <span className="font-semibold text-indigo-600 ml-1">{feedback.targetName || feedback.target_id || 'N/A'}</span></p>
    <p className="text-gray-700">{feedback.message}</p>
    <p className="text-xs text-gray-400 mt-4 text-right">Submitted on {new Date(feedback.created_at).toLocaleString()}</p>
  </motion.li>
);

function SuperAdminViewFeedbackPage() {
  ...
  const fetchFeedback = async () => {
      // ... fetchFeedback logic is unchanged ...
      setLoading(true);
      const supabase = await createClerkSupabaseClient(getToken);
      const [feedbackRes, clubsRes] = await Promise.all([
          supabase.from('feedback').select(`*, profile:profiles!feedback_submitter_id_fkey(full_name)`).order('created_at', { ascending: false }),
          supabase.from('clubs').select('id, club_name')
      ]);
      const { data: rawFeedback, error } = feedbackRes;
      const { data: clubs, error: clubError } = clubsRes;
      if (error || clubError) { toast.error("Failed to load data."); setLoading(false); return; }
      const clubMap = Object.fromEntries(clubs.map(c => [c.id, c.club_name]));
      const enrichedFeedback = rawFeedback.map(f => {
          let targetName = null;
          if (f.channel === 'club') { targetName = clubMap[f.target_id] || `Deleted Club (ID: ${f.target_id})`; } 
          else if (f.channel === 'hod') { targetName = `HOD of ${f.target_id}`; } 
          else { targetName = 'Super Admin'; }
          return { ...f, targetName };
      });
  ...
  useEffect(() => { fetchFeedback(); }, [getToken]);
  ...
  const handleSummarize = async () => {
      ...
      try {
          const allMessages = feedback.map(f => `[Feedback for ${f.targetName}]: ${f.message}`).join("\n---\n");
          const prompt = `You are a helpful assistant for a college administrator. Based on the following student feedback entries, provide a concise, neutral summary. Identify the key positive themes, the main points of criticism or concern, and any actionable suggestions. Structure your summary with clear headings:\n\n${allMessages}`;
    ...
    <>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h1 className="text-4xl font-bold text-gray-900">Feedback Inbox</h1>
              <p className="mt-2 text-lg text-gray-600">All feedback submitted across the platform.</p>
            </div>
        ...
        {loading ? (
          ...
          <ul role="list" className="space-y-4">
            {feedback.map((item, index) => <FeedbackCard key={item.id} feedback={item} index={index} onDelete={handleDelete} />)}
          </ul>
        ) : (
          <div className="text-center py-20 bg-white rounded-lg shadow-md">
             <Inbox className="mx-auto h-12 w-12 text-gray-400" /><h3 className="mt-2 text-xl font-semibold text-gray-700">Inbox is Clear</h3><p className="mt-1 text-gray-500">No feedback to review.</p>
          </div>
        )}
      </motion.div>
    </>
...
export default SuperAdminViewFeedbackPage;