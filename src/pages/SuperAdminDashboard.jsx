...
const InviteUserForm = () => {
    // ... InviteUserForm is correct and unchanged
    const { getToken } = useAuth();
    const [email, setEmail] = useState('');
    const [role, setRole] = useState('hod');
    const [loading, setLoading] = useState(false);
    const handleInviteUser = async (e) => {
        e.preventDefault(); setLoading(true); const toastId = toast.loading('Sending invitation...');
        try {
            ...
            toast.success(`Invitation sent to ${email}!`, { id: toastId }); setEmail('');
        } catch (error) {
    ...
    return (
        <div className="bg-white p-8 rounded-xl shadow-lg">
            <div className="flex items-center gap-4"><div className="bg-indigo-100 p-3 rounded-full"><UserPlus className="h-6 w-6 text-indigo-600" /></div><div><h2 className="text-2xl font-bold text-gray-800">Invite New User</h2><p className="text-gray-500">Send an invitation to a new HOD or Club Admin.</p></div></div>
...
const ClubManagement = () => {
    // ... ClubManagement is correct and unchanged
    const { getToken } = useAuth();
    const [clubName, setClubName] = useState('');
    const [clubAdmins, setClubAdmins] = useState([]);
    const [selectedAdmin, setSelectedAdmin] = useState('');
    const [loading, setLoading] = useState(false);
    const fetchClubAdmins = useCallback(async () => { if (!getToken) return; const supabase = await createClerkSupabaseClient(getToken); const { data, error } = await supabase.from('profiles').select('user_id, full_name').eq('role', 'club_admin'); if (error) { toast.error("Could not fetch club admins."); } else { setClubAdmins(data); } }, [getToken]);
    useEffect(() => { fetchClubAdmins(); }, [fetchClubAdmins]);
    const handleCreateClub = async (e) => {
        e.preventDefault(); setLoading(true); const toastId = toast.loading('Creating club...');
        try {
            const supabase = await createClerkSupabaseClient(getToken);
            const { error } = await supabase.from('clubs').insert({ club_name: clubName, club_admin_id: selectedAdmin || null });
            if (error) throw error;
            toast.success(`Club "${clubName}" created!`, { id: toastId }); setClubName(''); setSelectedAdmin('');
        } catch (error) {
...
function SuperAdminDashboard() {
  const contentOversightActions = [
      { to: "/lost-and-found", icon: Search, title: "Lost & Found", description: "Moderate lost & found items.", color: "bg-blue-500" },
      { to: "/notes", icon: FileText, title: "Notes & Papers", description: "Moderate shared notes.", color: "bg-yellow-500" },
      { to: "/club-feed", icon: Rss, title: "Club Feed", description: "Moderate club posts.", color: "bg-purple-500" },
  ];
